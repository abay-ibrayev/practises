...
class ProcurementItemDto {
    ...
    protected void copyPropertiesFrom(ProcurementItem item, Locale locale) {
        ...
        gswShortDescription = item.gswShortDescription
        gswAdditionalDescription = item.gswAdditionalDescription
        period = extractPeriod(item.startDate, item.finishDate, locale)
        marketingUnitPrice = item.marketingUnitPrice?.amount.toString()
        itemAmount = item.itemAmount
        totalCost = item.totalCost?.amount.toString()
    }
